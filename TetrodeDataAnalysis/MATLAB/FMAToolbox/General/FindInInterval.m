function [indices] = FindInIntervals(values,interval,from)
     i = find(values(:,1)>=interval(1)&values(:,1)<=interval(2));  % Laure: modified as does not seem to agree to use c function ?
     indices = [i(1),i(end)];
