function filtered = Filter(nyquist,samples,varargin)
...
switch(type),
    ...
    case 'butterwrth',
		if ~isempty(passband),
			if passband(1) == 0,
				[b, a] = butter(order, passband(2)/nyquist, 'low');  % Use Butterworth instead
			else
				[b, a] = butter(order, passband/nyquist, 'bandpass'); % Bandpass Butterworth
			end
		else
			[b, a] = butter(order, stopband/nyquist, 'stop'); % Stopband Butterworth
        end
	case 'fir1',
		if ~isempty(passband),
			if passband(1) == 0,
