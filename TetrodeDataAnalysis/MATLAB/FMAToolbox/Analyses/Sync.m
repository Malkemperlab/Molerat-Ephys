...
% Partially debugged and modified by Fabrizio Capitano (01/2018)

% Default values
samples=samples/1000; % convert to sec
sync=sync/1000;
% durations = [5 2];
durations = [-0.5 0.5];
verbose = false;
...
for i = 1:nSync,
    ...
    j = find(samples(:,1)>=sync(i)+durations(1) & samples(:,1)<=sync(i)+durations(2));
%      indices = [i(1),i(end)]; % ????
